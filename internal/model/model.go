...
func NewINotifyGoDAO(dbCfg config.DBConfig) INotifyGoDAO {
	engine, err := xorm.NewEngine(dbCfg.DriverName, dbCfg.Dsn)
	if err != nil {
		return nil
	}
...
func NewITemplateDAO(dbCfg config.DBConfig) ITemplateDAO {
	engine, err := xorm.NewEngine(dbCfg.DriverName, dbCfg.Dsn)
	if err != nil {
		return nil
	}
